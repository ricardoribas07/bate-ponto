...
// Rotas usadas pelo ESP32 (autenticadas por device_token, não por login)
app.use('/api/device', deviceRoutes);

// Rotas do dashboard (autenticadas por login de professor/coordenação)
app.use('/api/rooms', requireAuth, roomsRoutes);
app.use('/api/alerts', requireAuth, alertsRoutes);
app.use('/api/reports', requireAuth, reportsRoutes);

// Cadastros (só coordenação)
app.use('/api/admin', requireAuth, requireCoordenacao, adminRoutes);

const BACKEND_VERSION = '1.4.0'; // atualize este número a cada nova versão entregue
...
async function start() {
  await initDb(); // cria as tabelas e popula os dados de exemplo (se o banco estiver vazio)
  app.listen(PORT, () => {
